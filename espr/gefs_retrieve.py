...
class GEFSRetrieve:
    ...
    def link_builder(self):
        ...
        base_link_no_ens_leadtime = f"{base_link}{''.join([f'{self.query_dict[n]}' for n in self.query_dict])}"
        date_link = f'&dir=%2Fgefs.{self.date_value}%2F{self.hour_value}%2Fatmos%2Fpgrb2sp25&'
        new_link = base_link + base_link_no_ens_leadtime + date_link
        ens_fhour_link = f"file={self.ensemble_dict['mean']}.t00z.pgrb2s.0p25.f000"
        return base_link_no_ens_leadtime
