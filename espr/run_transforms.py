...
def run_mcli():
    mcli = mc.MClimate(
        datetime.today().strftime("%Y-%m-%d"), "/home/taylorm/espr/reforecast", "slp"
    )
...
def main(date, hour):
    ...
    fmean.to_netcdf(
        f'{paths["output"]}/slp_mean_{date.year}{date.month:02}{date.day:02}_{date.hour:02}z.nc'
    )
    fsprd.to_netcdf(
        f'{paths["output"]}/slp_sprd_{date.year}{date.month:02}{date.day:02}_{date.hour:02}z.nc'
    )
    ...
    ssa_perc.to_netcdf(
        f'{paths["output"]}/ssa_perc_{date.year}{date.month:02}{date.day:02}_{date.hour:02}z.nc'
    )
    hsa_final.to_netcdf(
        f'{paths["output"]}/hsa_{date.year}{date.month:02}{date.day:02}_{date.hour:02}z.nc'
    )
